...
class BalancedBatchSampler(Sampler):
    def __init__(self, labels, batch_size=32, minority_ratio=0.25):
        ...
        # Identify minority class (lowest frequency)
        counts = np.bincount(self.labels)
        self.minority_class = counts.argmin()

        self.minority_idx = np.where(self.labels == self.minority_class)[0].tolist()
        self.majority_idx = np.where(self.labels != self.minority_class)[0].tolist()

        self.minority_per_batch = int(batch_size * minority_ratio)
        self.majority_per_batch = batch_size - self.minority_per_batch

    def __iter__(self):
        np.random.shuffle(self.minority_idx)
        np.random.shuffle(self.majority_idx)

        m, M = 0, 0

        while True:
            if m + self.minority_per_batch > len(self.minority_idx):
                np.random.shuffle(self.minority_idx)
                m = 0

            if M + self.majority_per_batch > len(self.majority_idx):
                np.random.shuffle(self.majority_idx)
                M = 0

            batch = (
                self.minority_idx[m:m+self.minority_per_batch] +
                self.majority_idx[M:M+self.majority_per_batch]
            )
            np.random.shuffle(batch)

            m += self.minority_per_batch
            M += self.majority_per_batch
...
def main():
    ...
    train_loader = DataLoader(train_dataset,num_workers=0, batch_sampler=sampler)
